...
from cache_replacement.policy_learning.cache.cache import BernoulliProcessStatistic, CacheAccess, EvictionDecision
...
class CacheSet(object):
    ...
    def __init__(self, set_id, num_cache_lines, eviction_policy, access_history_len=30, parent_cache=None):
        """Constructs.
    ...
    def read(self, pc, address, observers=None):
        ...
        def update_cache_set(cache_access):
            ...
            self._parent_cache.last_evicted_cache_line = line_to_evict
        ...
        cache_access = CacheAccess(
            pc, address, self._set_id, list(self._cache_lines.items()), list(self._access_history)
        )
    ...
    def __str__(self):
        cache_lines = [str((hex(line), hex(pc))) for line, pc in self._cache_lines.items()]
...
class Cache(object):
    ...
    def from_config(cls, config, eviction_policy=None, trace=None, hit_rate_statistic=None):
        ...
        def eviction_policy_from_config(config, trace):
            ...
            def scorer_from_config(config, trace):
                ...
                if scorer_type == "lru":
                    ...
                    with open(config.get("config_path"), "r") as model_config:
                        return model_eviction_policy_mod.LearnedScorer.from_model_checkpoint(
                            cfg.Config.from_file(model_config), config.get("checkpoint")
                        )
                else:
            ...
            if policy_type == "greedy":
                ...
                return eviction_policy_mod.GreedyEvictionPolicy(scorer, config.get("n", 0))
            elif policy_type == "random":
                return eviction_policy_mod.RandomPolicy()
            elif policy_type == "mixture":
                subpolicies = [eviction_policy_from_config(subconfig, trace) for subconfig in config.get("subpolicies")]
                return eviction_policy_mod.MixturePolicy(subpolicies, config.get("weights"))
            else:
                raise ValueError("Invalid policy type: {}".format(policy_type))

        if eviction_policy is None:
            eviction_policy = eviction_policy_from_config(config.get("eviction_policy"), trace)
        return cls(
            config.get("capacity"),
            eviction_policy,
            config.get("associativity"),
    ...
    def __init__(
        ...
        if not is_pow_of_two(cache_line_size):
            raise ValueError("Cache line size ({}) must be a power of two.".format(cache_line_size))
        ...
        if cache_capacity % cache_line_size != 0 or num_cache_lines % associativity != 0:
            raise ValueError(
                (
                    "Cache capacity ({}) must be an even multiple of " "cache_line_size ({}) and associativity ({})"
                ).format(cache_capacity, cache_line_size, associativity)
            )
        ...
        if num_sets == 0:
            raise ValueError(
                (
                    "Cache capacity ({}) is not great enough for {} cache lines per set " "and cache lines of size {}"
                ).format(cache_capacity, associativity, cache_line_size)
            )
        ...
        self._sets = [
            CacheSet(set_id, associativity, eviction_policy, access_history_len, self) for set_id in range(num_sets)
        ]
        ...
        self.last_evicted_cache_line = None
    ...
    def clone(self, eviction_policy=None):
        assert len(self._sets) > 0, "Cache must have at least one set"
        old_policy = self._sets[0]._eviction_policy
        self.set_eviction_policy(None)
        cloned = deepcopy(self)  # replacement policy is not copied (may have open files which are not copyable)
        self.set_eviction_policy(old_policy)
        cloned.set_eviction_policy(eviction_policy)
        return cloned
