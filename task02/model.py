...
class EvictionPolicyModel(nn.Module):
  ...
  # NOTE: need to change inputs from LSTM to MLP
  def __init__(self, pc_embedder, address_embedder, cache_line_embedder,
    ...
    self._cache_pc_embedder = pc_embedder # cache_pc_embedder
    '''
    ...
    '''
    # Insert MLP here:
    self._MLP1 = nn.Linear(pc_embedder.embed_dim + address_embedder.embed_dim, 1)
    # self._softmax = nn.Softmax()
    # f(h, e(l))
    # NOTE: where is softmax of cache_line_scorer?
    '''
  ...
  def forward(self, cache_accesses, _, inference=False):
...
# Embed cache lines   
    ...
    # Flatten and concatenate cache_pc_embeddings
    cache_pc_embeddings = cache_pc_embeddings.view(32,-1)
# MLP Layer
    mlp_outputs = self._MLP1(torch.cat((pc_embedding,cache_pc_embeddings), -1))

    # (batch_size, num_cache_lines)
    scores = F.softmax(mlp_outputs.squeeze(-1), -1)
...
class ReuseDistanceLoss(LossFunction):
  ...
  def __call__(self, probs, predicted_log_reuse_distances,
    ...
    predicted_log_reuse_distances = predicted_log_reuse_distances.float()
    true_log_reuse_distances = true_log_reuse_distances.float()
    mask = mask.float()
