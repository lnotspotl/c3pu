...
class CacheEvictionMetric(abc.ABC):
    ...
    def write_to_tensorboard(self, tb_writer, tb_tag, step):
        ...
        Args:
          tb_writer (Writer): tensorboard writer to write to.
...
class SuccessRateMetric(CacheEvictionMetric):
    ...
    def update(self, prediction_probs, eviction_mask, oracle_scores):
        ...
        for i in range(self._k):
            top_i_successes = prediction_probs[:, 0] >= sorted_probs[:, min(i, num_elems - 1)]
    ...
    def write_to_tensorboard(self, tb_writer, tb_tag, step):
        for key in self._num_top_i_successes:
            for i in range(self._k):
                top_i_success_rate = self._num_top_i_successes[key][i] / (self._num_accesses[key] + 1e-8)
                utils.log_scalar(tb_writer, "{}/{}_top_{}".format(tb_tag, key, i + 1), top_i_success_rate, step)
...
class KendallWeightedTau(CacheEvictionMetric):
    ...
    def update(self, prediction_probs, eviction_mask, oracle_scores):
        ...
        for unbatched_order in predicted_order.cpu().data.numpy():
            ...
            weighted_tau, _ = stats.weightedtau(-unbatched_order, -np.array(range(len(unbatched_order))), rank=False)
    ...
    def write_to_tensorboard(self, tb_writer, tb_tag, step):
        ...
        eviction_mean_weighted_tau = np.sum(weighted_taus * eviction_masks) / (np.sum(eviction_masks) + 1e-8)
        utils.log_scalar(tb_writer, "{}/eviction_weighted_tau".format(tb_tag), eviction_mean_weighted_tau, step)

        utils.log_scalar(tb_writer, "{}/total_weighted_tau".format(tb_tag), np.mean(weighted_taus), step)
...
class OracleScoreGap(CacheEvictionMetric):
    ...
    def update(self, prediction_probs, eviction_mask, oracle_scores):
        ...
        self._optimal_scores.append([scores[0] if len(scores) > 0 else 1 for scores in oracle_scores])
        self._evicted_scores.append(
            [scores[index] if len(scores) > 0 else 1 for (index, scores) in zip(chosen_indices, oracle_scores)]
        )
        self._masks.append(eviction_mask.cpu().data.numpy())

    def write_to_tensorboard(self, tb_writer, tb_tag, step):
        eviction_masks = np.array(self._masks)
        difference_gap = np.log10(np.array(self._evicted_scores) - np.array(self._optimal_scores) + 1)
        quotient_gap = np.log10(np.array(self._optimal_scores) / np.array(self._evicted_scores))
        gaps = {
        ...
        for gap_type, gap in gaps.items():
            eviction_mean_gap = np.sum(gap * eviction_masks) / (np.sum(eviction_masks) + 1e-8)
            utils.log_scalar(
                tb_writer, "{}/eviction_oracle_score_{}_gap".format(tb_tag, gap_type), eviction_mean_gap, step
            )
            utils.log_scalar(tb_writer, "{}/oracle_score_{}_gap".format(tb_tag, gap_type), np.mean(gap), step)
