...
def from_config(config):
    ...
    if embedder_type == "byte":
        ...
        return DynamicVocabEmbedder(config.get("embed_dim"), config.get("max_vocab_size"))
...
class ByteEmbedder(Embedder):
    ...
    def __init__(self, bytes_per_entry, embed_dim):
        ...
        if embed_dim % bytes_per_entry != 0:
            raise ValueError(
                "Embed dim ({}) must be an even multiple of bytes per entry ({})".format(embed_dim, bytes_per_entry)
            )
...
class DynamicVocabEmbedder(Embedder):
    ...
    def forward(self, inputs):
        ...
        def input_to_index(inp):
            if inp not in self._input_to_index and self._max_vocab_size > self._vocab_size:
...
class PositionalEmbedder(Embedder):
    ...
    def forward(self, position_indices):
        ...
        embedding = position_tensor / 10000.0 ** (2 * embed_indices / self.embed_dim)
        embedding = torch.where(embed_indices % 2 == 0, torch.sin(embedding), torch.cos(embedding))
        return embedding
