...
def approx_ndcg(scores, relevances, alpha=10.0, mask=None):
    ...
    def approx_positions(scores, alpha=10.0):
        # s_{i, j} (batch_size, num_elems)
        diff = scores.unsqueeze(-1).expand(-1, -1, scores.shape[1]) - scores.unsqueeze(1).expand(
            -1, scores.shape[1], -1
        )
    ...
    idcg = torch.expm1(relevances) * mask.float() / torch.log1p(torch.arange(scores.shape[-1]).float() + 1)
