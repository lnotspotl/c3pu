...
class ScaledDotProductAttention(Attention):
    ...
    def __init__(self, memory_key_dim):
        ...
        """
        super().__init__()
        self._scale = 1.0 / np.sqrt(memory_key_dim)
...
class MultiQueryAttention(nn.Module):
    ...
    def forward(self, memory_keys, memory_values, queries, masks=None):
        ...
        memory_keys = memory_keys.repeat(1, num_queries, 1).view(batch_size * num_queries, num_cells, -1)
        memory_values = memory_values.repeat(1, num_queries, 1).view(batch_size * num_queries, num_cells, -1)
        masks = masks.repeat(1, num_queries).view(batch_size * num_queries, num_cells)
