...
class GreedyEvictionPolicy(EvictionPolicy):
    ...
    def __call__(self, cache_access, access_times):
        ...
        sorted_cache_lines = heapq.nsmallest(self._n + 1, scores.keys(), key=lambda line: scores[line])
...
class MixturePolicy(EvictionPolicy):
    ...
    def __init__(self, policies, weights=None, seed=0, scoring_policy_index=None):
        ...
        if len(policies) != len(weights):
            raise ValueError(
                "Need the same number of weights ({}) as policies ({})".format(len(policies), len(weights))
            )
...
class LRUScorer(CacheLineScorer):
    ...
    def __call__(self, cache_access, access_times):
        scores = {line: access_times[line] for (line, _) in cache_access.cache_lines}
        return scores
...
class BeladyScorer(CacheLineScorer):
    ...
    def __call__(self, cache_access, access_times):
        ...
        scores = {line: -self._memtrace.next_access_time(line) for (line, _) in cache_access.cache_lines}
        return scores
