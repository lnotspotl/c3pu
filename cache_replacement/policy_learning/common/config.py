...
class Config(object):
    ...
    def merge(cls, configs):
        ...
        for config in configs[1:]:
            for key in config.keys():
                ...
                if not isinstance(current_value, Config) or not isinstance(merging_value, Config):
    ...
    def set(self, key, value):
        ...
        if current_value is not None and not isinstance(current_value, Config):
            raise ValueError(
                ("Trying to perform nested set with key: {}, " "but value associated with {} is not nested.").format(
                    key, nested_keys[0]
                )
            )

        sub_config = self.__class__(self._config_tree.setdefault(nested_keys[0], {}))
