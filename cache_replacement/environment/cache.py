...
class Cache(object):
    ...
    def from_config(cls, config):
        ...
        Returns:
          Cache
        """
        return cls(config.get("capacity"), config.get("associativity"), config.get("cache_line_size"))

    def __init__(self, cache_capacity, associativity, cache_line_size=64, child_cache=None):
    ...
    def access(self, address):
        ...
        if evict and self._child_cache is not None:
            _, _, children_evicts, children_lines, children_set_ids = self._child_cache.access(address)
