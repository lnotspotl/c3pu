...
class State(collections.namedtuple("State", ("access", "cache_lines", "access_history", "set_id", "evict"))):
...
class CacheReplacementEnv(gym.Env):
    ...
    def reset(self):
        ...
        self._memtrace = memtrace_mod.MemoryTrace(
            self._memtrace_filename, cache_line_size=self._cache_config.get("cache_line_size")
        ).__enter__()
        self._cache = cache_mod.Cache.from_config(self._cache_config)
        self._set_access_history = collections.defaultdict(lambda: collections.deque(maxlen=self._access_history_len))

        pc, address = self._memtrace.next()
        aligned_address, _, evicts, lines, set_ids = self._cache.access(address)
        self._prev_state = State(
            access=(aligned_address, pc), cache_lines=lines[0], access_history=[], set_id=set_ids[0], evict=evicts[0]
        )
    ...
    def step(self, action):
        if self._prev_state is not None:
            if action not in self._prev_state.action_set:
                raise ValueError("Permitted actions: {}. Taken action: {}".format(self._prev_state.action_set, action))
