...
class BeladyPolicy(policy.ReplacementPolicy):
    ...
    def action(self, state):
        ...
        reuse_distances = {line: self._env.next_access_time(line) for line in state.cache_lines}
        line_to_evict = max((reuse_distance, line) for line, reuse_distance in reuse_distances.items())[1]
...
class BeladyNearestNeighborsPolicy(policy.ReplacementPolicy):
    ...
    def __init__(self, train_env):
        ...
        with tqdm.tqdm(desc='"Training"') as pbar:
            while True:
                reuse_distances = {line: train_env.next_access_time(line) for line in state.cache_lines}

                # Ranked highest reuse distance to lowest reuse distance
                ranked_lines = sorted(state.cache_lines, key=lambda line: reuse_distances[line], reverse=True)
                self._address2index[(state.set_id, state.access)].append(len(self._train_accesses[state.set_id]))
    ...
    def action(self, state):
        ...
        longest_suffix_index = max(self._address2index[(state.set_id, state.access)], key=suffix_len, default=0)
        ...
        train_decisions = self._train_accesses[state.set_id][longest_suffix_index][1]
        try:
