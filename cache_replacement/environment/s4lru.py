...
class SegmentedQueue(object):
    ...
    def __init__(self, cache_size, num_queues=4):
        ...
        self._queues = [collections.deque(maxlen=self._max_queue_size) for _ in range(num_queues)]
    ...
    def __str__(self):
        return "\n".join(f"level {level}: {queue}" for level, queue in enumerate(self._queues))
...
class S4LRU(policy.ReplacementPolicy):
    ...
    def __init__(self, cache_set_size):
        ...
        self._queues = collections.defaultdict(lambda: SegmentedQueue(cache_set_size))
