...
class EvictionPolicyModel(nn.Module):
    ...
    def __init__(
        ...
        self._lstm_cell = nn.LSTMCell(pc_embedder.embed_dim + address_embedder.embed_dim + 2*byte_embedder.embed_dim, lstm_hidden_size) # +2 for additional integer features
...
# Extract features
        #hit_count = torch.tensor([int(cache_access.features[10]) for cache_access in cache_accesses]).unsqueeze(-1)
        #SALM = torch.tensor([int(cache_access.features[11]) for cache_access in cache_accesses]).unsqueeze(-1)
        

# Embed features
        hit_count_embedding = self._byte_embedder([int(cache_access.features[10]) for cache_access in cache_accesses])
        SALM_embedding = self._byte_embedder([int(cache_access.features[11]) for cache_access in cache_accesses])

        # Each (batch_size, hidden_size)
        next_c, next_h = self._lstm_cell(torch.cat((pc_embedding, address_embedding, hit_count_embedding, SALM_embedding), -1), hidden_state)
