...
from HW3FeatureEngineering.HC_SALM import cache as cache_mod
from cache_replacement.policy_learning.cache import eviction_policy
from HW3FeatureEngineering.HC_SALM import evict_trace
from HW3FeatureEngineering.HC_SALM import memtrace
from HW3FeatureEngineering.HC_SALM import eviction_policy as model_eviction_policy
from cache_replacement.policy_learning.cache_model import metric, schedules, utils
from HW3FeatureEngineering.HC_SALM import model
...
flags.DEFINE_string(
    "train_memtrace",
    "cache_replacement/policy_learning/cache/traces/astar_313B_test_features.csv",
...
flags.DEFINE_integer("total_steps", int(1e6), "Number of training steps to take.")
flags.DEFINE_integer("tb_freq", 100, "Steps between logging to tensorboard.")
flags.DEFINE_integer("small_eval_size", 30000, "Number of examples to evaluate on in small evaluations.")
flags.DEFINE_integer(
    "small_eval_freq", 4000, "Steps between evaluating on small_eval_size examples of validation data."
)
flags.DEFINE_integer("full_eval_freq", 48000, "Steps between evaluating on ALL of validation data.")
flags.DEFINE_integer("save_freq", 20000, "Steps between saving model checkpoints.")
flags.DEFINE_integer(
...
flags.DEFINE_multi_string(
    "model_configs",
    ["HW3FeatureEngineering/small_byte.json"],  # pylint: disable=line-too-long
    ("List of config filenames merged front to back for the model. " "See config.Config.merge for merging behavior."),
)
flags.DEFINE_multi_string("model_bindings", [], "bindings to override model config.")
flags.DEFINE_multi_string(
    "cache_configs",
...
flags.DEFINE_multi_string("cache_bindings", [], "bindings to override cache config.")
flags.DEFINE_multi_string(
...
flags.DEFINE_multi_string("dagger_schedule_bindings", [], "bindings to override DAgger config.")
flags.DEFINE_enum("oracle_eviction_policy", "belady", ["lru", "belady"], "Which eviction policy to learn.")
flags.DEFINE_integer("seed", 0, "Determines which random seed to use.")
flags.DEFINE_bool(
    "fast_train", False, "If true, the training process will not perform any evaluations, just train the model."
)


def schedule_from_config(config):
    if config.get("type") == "linear":
        return schedules.LinearSchedule(config.get("num_steps"), config.get("final"), config.get("initial"))
...
def evaluate(policy_model, data, step, descriptor, tb_writer, log_dir, k=5):
    ...
    def pretty_print(entry, probs, attention, pred_reuse_distances):
        ...
        s = [
            "PC: {}\nAddress: {}\nEvict: {}\nCache lines:\n".format(
                hex(cache_access.pc), hex(cache_access.address), eviction_decision.evict
            )
        ]

        _, true_ranks = probs.sort(descending=True)
        true_rank_to_pred_rank = dict(zip(true_ranks.cpu().data.numpy(), range(len(true_ranks))))
        headers = [
            "true rank",
            "pc",
            "address",
        ...
        for i, (attention_weights, access) in enumerate(reversed(attention)):
            ...
            attention_entries = ["{:.2f}".format(weight) for weight in attention_weights[:num_cache_lines]]
            row = ["t - {}".format(i), hex(access.pc), hex(access.address)] + attention_entries
    ...
    subsequences = [data[i * subseq_length : (i + 1) * subseq_length] for i in range(FLAGS.batch_size)]
    ...
    for batch in tqdm.tqdm(zip(*subsequences), desc=desc, total=subseq_length):
        ...
        eviction_mask = torch.tensor([entry.eviction_decision.evict for entry in batch])
        oracle_scores = []
        for entry in batch:
            # Set min score to -1e10 to handle -np.infs
            oracle_scores.append(
                np.maximum(
                    [entry.eviction_decision.cache_line_scores[line] for line, _ in entry.cache_access.cache_lines],
                    -1e10,
                )
            )
        for m in metrics:
...
def measure_cache_hit_rate(
    ...
    with memtrace.MemoryTrace(memtrace_path, cache_line_size=line_size) as trace:
        ...
        def create_eviction_policy(model_prob):
            ...
            if eval:
                oracle_scorer = oracle_scorers["belady"]
            else:
                oracle_scorer = oracle_scorers[FLAGS.oracle_eviction_policy]
        ...
        cache = cache_mod.Cache.from_config(cache_config, eviction_policy=policy, trace=trace)
            ...
            while not trace.done():
                ...
                with evict_trace.EvictionTrace(eviction_trace_path, False) as etrace:
                    ...
                    while len(data) < max_examples and not trace.done():
                        pc, address, features = trace.next()
                        pcs.add(pc)
                        cache.read(pc, address, features, [add_to_data])
...
def log_hit_rates(tb_writer, tb_key, hit_rates, step):
    ...
    for i, hit_rate in enumerate(hit_rates[:-1]):
        utils.log_scalar(tb_writer, tb_key + "_{:.2f}".format((i + 1) / len(hit_rates)), hit_rate, step)
...
def main(_):
    ...
    model_config = cfg.Config.from_files_and_bindings(FLAGS.model_configs, FLAGS.model_bindings)
    logging.info("Model config: %s", model_config)
    with open(os.path.join(exp_dir, "model_config.json"), "w") as f:
        model_config.to_file(f)

    cache_config = cfg.Config.from_files_and_bindings(FLAGS.cache_configs, FLAGS.cache_bindings)
    ...
    with tqdm.tqdm(total=FLAGS.total_steps) as pbar:
        while True:  # loop for waiting until steps == FLAGS.total_steps
            ...
            max_examples = dagger_schedule_config.get("update_freq") * FLAGS.collection_multiplier * FLAGS.batch_size
            ...
            for train_data, hit_rates in train_data_generator:
                ...
                utils.log_scalar(tb_writer, "cache_hit_rate/mixture_parameter", dagger_schedule.value(step), step)

                for batch_num, batch in enumerate(
                    utils.as_batches([train_data], FLAGS.batch_size, model_config.get("sequence_length"))
                ):
                    ...
                    def evaluate_helper(eval_size, suffix):
                        ...
                        evaluate(
                            policy_model,
                            oracle_valid_data[-eval_size:],
                            step,
                            "off_policy_valid" + suffix,
                            tb_writer,
                            predictions_dir,
                        )
                        ...
                        evaluate(
                            policy_model, train_data[-eval_size:], step, "train" + suffix, tb_writer, predictions_dir
                        )
                        ...
                        _, hit_rates = next(
                            measure_cache_hit_rate(
                                ...
                                get_step,
                                os.path.join(evict_trace_dir, "train{}-{}.txt".format(suffix, step)),
                                max_examples=eval_size,
                                use_oracle_scores=False,
                            )
                        )
                        log_hit_rates(tb_writer, "cache_hit_rate/train" + suffix, hit_rates, step)
                        ...
                        on_policy_valid_data, hit_rates = next(
                            measure_cache_hit_rate(
                                ...
                                schedules.ConstantSchedule(1),
                                get_step,
                                os.path.join(evict_trace_dir, "valid{}-{}.txt".format(suffix, step)),
                                max_examples=eval_size,
                            )
                        )
                        log_hit_rates(tb_writer, "cache_hit_rate/valid" + suffix, hit_rates, step)
                        evaluate(
                            policy_model,
                            on_policy_valid_data[-eval_size:],
                            step,
                            "on_policy_valid" + suffix,
                            tb_writer,
                            predictions_dir,
                        )

                    if not FLAGS.fast_train and step % FLAGS.small_eval_freq == 0:
                        evaluate_helper(FLAGS.small_eval_size, "")

                    if not FLAGS.fast_train and step % FLAGS.full_eval_freq == 0:
                    ...
                    optimizer.zero_grad()
                    losses = policy_model.loss(batch, model_config.get("sequence_length") // 2)
                    ...
                    if step % FLAGS.tb_freq == 0:
                        ...
                        for loss_name, loss_value in losses.items():
                            utils.log_scalar(tb_writer, "loss/{}".format(loss_name), loss_value, step)
