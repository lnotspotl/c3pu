...
from HW3FeatureEngineering.HC_SALM import eviction_policy as model_eviction_policy_mod
...
class CacheSet(object):
    ...
    def read(self, pc, address, features, observers=None):
        ...
        cache_access = CacheAccess(
            pc, address, features ,self._set_id, list(self._cache_lines.items()), list(self._access_history)
        )
...
class Cache(object):
    ...
    def __init__(
        ...
        self._sets = [
            CacheSet(set_id, associativity, eviction_policy, access_history_len) for set_id in range(num_sets)
        ]
    ...
    def read(self, pc, address, features, observers=None):
        ...
        hit = self._sets[set_id].read(pc, aligned_address, features, observers=observers)
...
class CacheAccess(collections.namedtuple("CacheAccess", ("pc", "address", "features", "set_id", "cache_lines", "access_history"))):
...
class EvictionDecision(collections.namedtuple("EvictionDecision", ("evict", "cache_line_scores"))):
    ...
    def rank_cache_lines(self, cache_lines):
        ...
        """
        return sorted(cache_lines, key=lambda cache_line: self.cache_line_scores[cache_line[0]])
