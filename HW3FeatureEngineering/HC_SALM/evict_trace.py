...
from HW3FeatureEngineering.HC_SALM import cache


class EvictionEntry(collections.namedtuple("EvictionEntry", ("cache_access", "eviction_decision"))):
    ...
    def __new__(cls, cache_access, eviction_decision):
        cache_access = cache_access._replace(cache_lines=eviction_decision.rank_cache_lines(cache_access.cache_lines))
        return super(EvictionEntry, cls).__new__(cls, cache_access, eviction_decision)
...
class EvictionTrace(object):
    ...
    def read(self):
        ...
        cache_lines = [(int(line, 16), int(pc, 16)) for line, pc in entry["cache_lines"]]
        access_history = [(int(address, 16), int(pc, 16)) for address, pc in entry["access_history"]]
        cache_access = cache.CacheAccess(pc, address, set_id, cache_lines, access_history)
        ...
        json.dump(
            {
                ...
                "cache_lines": [(hex(line), hex(pc)) for line, pc in entry.cache_access.cache_lines],
                "access_history": [(hex(address), hex(pc)) for address, pc in entry.cache_access.access_history],
                "evict": entry.eviction_decision.evict,
                # Serialize items instead of dict to prevent json from converting int
                # keys to string
                "cache_line_scores": list(entry.eviction_decision.cache_line_scores.items()),
            },
            self._file,
        )
        self._file.write("\n")
