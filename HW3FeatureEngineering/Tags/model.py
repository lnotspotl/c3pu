...
class EvictionPolicyModel(nn.Module):
    ...
    def from_config(cls, config):
        ...
        byte_embedder = embed.from_config(config.get("byte_embedder"))
    ...
    def __init__(
        ...
        self._lstm_cell = nn.LSTMCell(pc_embedder.embed_dim + 3*byte_embedder.embed_dim, lstm_hidden_size) # +3 for additional integer features
        self._positional_embedder = positional_embedder
        self._byte_embedder = byte_embedder
...
# Extract features
        #tag1 = torch.tensor([int(cache_access.features[7], 16) for cache_access in cache_accesses]).unsqueeze(-1)
        #tag2 = torch.tensor([int(cache_access.features[8], 16) for cache_access in cache_accesses]).unsqueeze(-1) 
        #tag3 = torch.tensor([int(cache_access.features[9], 16) for cache_access in cache_accesses]).unsqueeze(-1) 

# Embed features
        tag1_embedding = self._byte_embedder([int(cache_access.features[7], 16) for cache_access in cache_accesses])
        tag2_embedding = self._byte_embedder([int(cache_access.features[8], 16) for cache_access in cache_accesses])
        tag3_embedding = self._byte_embedder([int(cache_access.features[9], 16) for cache_access in cache_accesses])

        # Each (batch_size, hidden_size)
        next_c, next_h = self._lstm_cell(torch.cat((pc_embedding, tag1_embedding, tag2_embedding, tag3_embedding), -1), hidden_state)
...
class LossFunction(abc.ABC):
    ...
    @abc.abstractmethod
    def __call__(self, probs, predicted_log_reuse_distances, true_log_reuse_distances, mask):
...
class KLLoss(LossFunction):
    ...
    def __call__(self, probs, predicted_log_reuse_distances, true_log_reuse_distances, mask):
        approx_oracle_policy = td.Categorical(logits=self._temperature * true_log_reuse_distances)
...
class ReuseDistanceLoss(LossFunction):
    ...
    def __call__(self, probs, predicted_log_reuse_distances, true_log_reuse_distances, mask):
        ...
        return F.mse_loss(
            ...
            true_log_reuse_distances.float() * mask.float(),
            reduce=False,
        ).mean(-1)
