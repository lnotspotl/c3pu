...
class MemoryTrace(object):
    ...
    def _read_next(self):
        ...
        try:
            pc, address, features = self._reader.next()
            self._look_ahead_buffer.append((pc, address, features))
            # Align to cache line
            self._access_times[address >> self._offset_bits].append(len(self._look_ahead_buffer) + self._num_next_calls)
    ...
    def next(self):
        ...
        pc, address, features = self._look_ahead_buffer.popleft()
        # Align to cache line
    ...
    def __enter__(self):
        ...
        if extension == ".csv":
            ...
            raise ValueError("Extension {} not a supported extension.".format(extension))

        # Initialize look-ahead buffer
        for _ in tqdm.tqdm(range(self._max_look_ahead), desc="Initializing MemoryTrace"):
            self._read_next()
        return self
...
class MemoryTraceReader(six.with_metaclass(abc.ABCMeta, object)):
    ...
    @abc.abstractmethod
    def next(self):
        """Returns the next (pc, address, feature) in the file.
...
class CSVReader(MemoryTraceReader):
    ...
    def next(self):
        ...
        pc , address, features = row[0], row[1], row[2:]
        
        #pc, address = next(self._csv_reader)
        # Convert hex string to int
        return int(pc, 16), int(address, 16), features
...
class TxtReader(MemoryTraceReader):
    ...
    def next(self):
        ...
        _, pc, address, feature = line.split()
        # Already in decimal
...
class MemoryTraceWriter(object):
    ...
    def write(self, pc, address, features):
        """Writes the (pc, address, features) to disk.

        Args:
          pc (int): program counter of instruction causing read at the address.
          address (int): memory address accessed in the instruction.
        """
        self._csv_writer.writerow([hex(pc), hex(address)] + features)
